...
async function quickDoneTask(id, currentStatus) {
  ...
  if (newStatus === 'done' && window.puBravo) window.puBravo();
}
...
function calcMileageAmount() {
  ...
  const rate = parseFloat(form.querySelector('[name=rate]')?.value) || mileageRateFor(form.querySelector('[name=user_name]')?.value);
...
async function loadDevisRequests() {
  ...
  const rows = !data.length ? emptyMsg : data.map(r => {
    ...
    return `<tr>
      ...
      <td style="display:flex;gap:4px">
        <a href="${ASSISTANT_URL}/?demande=${r.id}" target="_blank" rel="noopener" style="text-decoration:none;font-size:1rem;padding:2px 6px" title="Faire ce devis dans l'Assistant (dossier pré-rempli)">✨</a>
  ...
  // Et les devis complets faits dans l'Assistant, juste en dessous
  loadDevisAssistant();
}

/* ============================================================
   LES DEVIS FAITS DANS L'ASSISTANT
   L'Assistant enregistre chaque devis (lignes, totaux, statut)
   dans la table Supabase « devis ». Ici on les affiche, on peut
   changer leur statut, et rouvrir le dossier dans l'Assistant.
   Si la table n'existe pas encore (script SQL pas exécuté),
   le bloc reste simplement masqué — aucune erreur.
   ============================================================ */
const ASSISTANT_URL = 'https://pullup-assistant-equipe.netlify.app';
const DEVIS_STATUTS = ['Brouillon', 'Envoyé', 'Relancé', 'Négociation', 'Accepté', 'Perdu'];
const DEVIS_STATUT_COLOR = {
  'Brouillon': '#aaa', 'Envoyé': '#4A9EFF', 'Relancé': '#FF9800',
  'Négociation': '#F5C518', 'Accepté': '#4CAF50', 'Perdu': '#f44336'
};

async function loadDevisAssistant() {
  const bloc = document.getElementById('devis-assistant-bloc');
  if (!bloc) return;
  const { data, error } = await sb.from('devis').select('*').order('updated_at', { ascending: false });
  if (error) { bloc.style.display = 'none'; return; }   // table absente ou vide d'accès : on se tait
  if (!data.length) { bloc.style.display = 'none'; return; }
  bloc.style.display = '';

  const esc = s => String(s ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/"/g, '&quot;');
  const eur = n => (parseFloat(n) || 0).toLocaleString('fr-FR', { maximumFractionDigits: 0 }) + ' €';

  // Petites tuiles : montant en jeu (envoyé/relancé/négo), accepté, transformation
  const enJeu = data.filter(d => ['Envoyé', 'Relancé', 'Négociation'].includes(d.statut));
  const acceptes = data.filter(d => d.statut === 'Accepté');
  const decides = data.filter(d => ['Accepté', 'Perdu'].includes(d.statut));
  const tuile = (val, label, color) => `
    <div style="background:var(--bg2);border-radius:12px;padding:12px 20px;min-width:140px">
      <div style="font-size:1.6rem;font-weight:700;color:${color}">${val}</div>
      <div style="font-size:.8rem;color:var(--text2)">${label}</div>
    </div>`;
  document.getElementById('devis-assistant-stats').innerHTML =
    tuile(eur(enJeu.reduce((s, d) => s + (parseFloat(d.total_ht) || 0), 0)), enJeu.length + ' devis en attente de réponse', 'var(--gold)') +
    tuile(eur(acceptes.reduce((s, d) => s + (parseFloat(d.total_ht) || 0), 0)), acceptes.length + (acceptes.length > 1 ? ' devis acceptés' : ' devis accepté'), '#4CAF50') +
    (decides.length ? tuile(Math.round(acceptes.length / decides.length * 100) + ' %', 'de devis gagnés', '#4A9EFF') : '');
  document.getElementById('devis-assistant-total').textContent = data.length + ' devis au total';

  document.getElementById('devis-assistant-tbody').innerHTML = data.map(d => {
    const sc = DEVIS_STATUT_COLOR[d.statut] || '#aaa';
    const nbLignes = Array.isArray(d.lignes) ? d.lignes.length : 0;
    const detail = nbLignes ? `title="${esc((d.lignes || []).map(l => l.nom + (l.qte > 1 ? ' ×' + l.qte : '')).join(' · '))}"` : '';
    return `<tr>
      <td style="font-weight:600">${esc(d.numero || '—')}</td>
      <td><strong>${esc(d.client || '—')}</strong>${d.contact_name ? '<br><span style="font-size:.78rem;color:var(--text2)">' + esc(d.contact_name) + '</span>' : ''}</td>
      <td ${detail}>${esc(d.event_type || '—')}${nbLignes ? ` <span style="font-size:.75rem;color:var(--text2)">(${nbLignes} lignes)</span>` : ''}</td>
      <td>${d.event_date ? new Date(d.event_date + 'T00:00:00').toLocaleDateString('fr-FR') : '—'}</td>
      <td style="text-align:center">${d.participants || '—'}</td>
      <td style="font-weight:700;color:var(--gold)">${d.total_ht ? eur(d.total_ht) : '—'}</td>
      <td>
        <select onchange="updateDevisAssistantStatut('${d.id}',this)" style="background:${sc}22;color:${sc};border:1px solid ${sc}55;border-radius:10px;padding:2px 8px;font-size:.75rem;font-weight:600;cursor:pointer;outline:none">
          ${DEVIS_STATUTS.map(s => `<option ${d.statut === s ? 'selected' : ''}>${s}</option>`).join('')}
        </select>
      </td>
      <td style="font-size:.8rem;color:var(--text2)">${d.updated_at ? new Date(d.updated_at).toLocaleDateString('fr-FR') : '—'}</td>
      <td>
        <a href="${ASSISTANT_URL}/?dossier=${encodeURIComponent(d.dossier_assistant || '')}" target="_blank" rel="noopener" style="text-decoration:none;font-size:1rem;padding:2px 6px" title="Ouvrir ce dossier dans l'Assistant">✨</a>
      </td>
    </tr>`;
  }).join('');
}

async function updateDevisAssistantStatut(id, sel) {
  const { error } = await sb.from('devis')
    .update({ statut: sel.value, updated_at: new Date().toISOString() }).eq('id', id);
  if (error) { showToast('Erreur : ' + error.message); return; }
  showToast('Statut du devis mis à jour — l\'Assistant le verra aussi');
  logAudit('Devis Assistant', `Statut « ${sel.value} » — devis ${id}`);
  loadDevisAssistant();
}
...
function renderBanqueMonthly(annee, finRows) {
  ...
  const byMonth = {};
  _banqueTx.forEach(t => {
    if (!byMonth[t.mois]) byMonth[t.mois] = { in: 0, out: 0 };
    byMonth[t.mois].in += parseFloat(t.credit) || 0;
    byMonth[t.mois].out += parseFloat(t.debit) || 0;
  });

  let totIn = 0, totOut = 0, totCa = 0, nbMois = 0;
  let html = '';
  for (let m = 1; m <= 12; m++) {
    const d = byMonth[m];
    const ca = caByMonth[m] || 0;
    if (!d && !ca) continue;
    const inM = d ? d.in : 0, outM = d ? d.out : 0;
    const net = inM - outM;
    if (d) { totIn += inM; totOut += outM; nbMois++; }
    totCa += ca;
    html += `<tr style="cursor:${d ? 'pointer' : 'default'}" ${d ? `onclick="showBanqueMois(${annee},${m})"` : ''}>
      <td style="font-weight:600">${MOIS_LABELS[m-1]} ${d ? '<span style="font-size:.7rem;color:var(--gold);opacity:.7">▶</span>' : ''}</td>
      <td style="color:#4CAF50;font-weight:700">${d ? Math.round(inM).toLocaleString('fr-FR') + ' €' : '—'}</td>
      <td style="color:#f44336;font-weight:700">${d ? Math.round(outM).toLocaleString('fr-FR') + ' €' : '—'}</td>
      <td style="font-weight:700;color:${net >= 0 ? '#4CAF50' : '#f44336'}">${d ? (net >= 0 ? '+' : '') + Math.round(net).toLocaleString('fr-FR') + ' €' : '—'}</td>
      <td style="color:var(--gold);font-weight:700">${ca ? Math.round(ca).toLocaleString('fr-FR') + ' €' : '—'}</td>
    </tr>`;
  }
  tbody.innerHTML = html || `<tr><td colspan="5" style="text-align:center;color:var(--text2);padding:1.2rem">Aucune opération bancaire pour ${annee}. Utilise le bouton « 📥 Importer un export Qonto » en haut.</td></tr>`;

  const el = id => document.getElementById(id);
  const totNet = totIn - totOut;
  if (el('banque-annual-in')) el('banque-annual-in').textContent = totIn ? Math.round(totIn).toLocaleString('fr-FR') + ' €' : '—';
  if (el('banque-annual-out')) el('banque-annual-out').textContent = totOut ? Math.round(totOut).toLocaleString('fr-FR') + ' €' : '—';
  if (el('banque-annual-net')) {
    el('banque-annual-net').textContent = (totIn || totOut) ? (totNet >= 0 ? '+' : '') + Math.round(totNet).toLocaleString('fr-FR') + ' €' : '—';
    el('banque-annual-net').style.color = totNet >= 0 ? '#4CAF50' : '#f44336';
  }
  if (el('banque-annual-ca')) el('banque-annual-ca').textContent = totCa ? Math.round(totCa).toLocaleString('fr-FR') + ' €' : '—';

  const kpis = document.getElementById('banque-kpis');
  if (kpis) {
    kpis.style.display = _banqueTx.length ? 'grid' : 'none';
    if (el('banque-kpi-in')) el('banque-kpi-in').textContent = Math.round(totIn).toLocaleString('fr-FR') + ' €';
    if (el('banque-kpi-out')) el('banque-kpi-out').textContent = Math.round(totOut).toLocaleString('fr-FR') + ' €';
    if (el('banque-kpi-net')) {
      el('banque-kpi-net').textContent = (totNet >= 0 ? '+' : '') + Math.round(totNet).toLocaleString('fr-FR') + ' €';
      el('banque-kpi-net').style.color = totNet >= 0 ? '#4CAF50' : '#f44336';
    }
    if (el('banque-kpi-avg')) el('banque-kpi-avg').textContent = nbMois ? Math.round(totOut / nbMois).toLocaleString('fr-FR') + ' €' : '—';
  }
}

function showBanqueMois(annee, mois) {
  const card = document.getElementById('banque-detail-card');
  if (!card) return;
  const txs = _banqueTx.filter(t => t.mois === mois);
  document.getElementById('banque-detail-title').textContent = `🔍 ${MOIS_LABELS[mois-1]} ${annee} — ${txs.length} opérations`;
  ...
  txs.forEach(t => {
    ...
    if (!deb) return;
    totOut += deb;
    cats[t.categorie || 'Autres'] = (cats[t.categorie || 'Autres'] || 0) + deb;
  });
  const catsSorted = Object.entries(cats).sort((a, b) => b[1] - a[1]);
  document.getElementById('banque-detail-cats').innerHTML = catsSorted.map(([cat, sum]) => {
    const pct = totOut ? Math.round((sum / totOut) * 100) : 0;
    return `<div style="background:var(--bg3);border-radius:10px;padding:10px 14px">
      <div style="display:flex;justify-content:space-between;font-size:.85rem;margin-bottom:6px">
        <span>${cat}</span><strong style="color:#f44336">${Math.round(sum).toLocaleString('fr-FR')} €</strong>
      </div>
      ...
      <div style="font-size:.7rem;color:var(--text2);margin-top:4px">${pct}% des sorties</div>
    </div>`;
  ...
  document.getElementById('banque-detail-liste').innerHTML = facHtml + txs.map(t => {
    ...
    return `<div style="display:flex;align-items:center;gap:10px;padding:7px 4px;border-bottom:1px solid var(--border);font-size:.85rem">
      <span style="color:var(--text2);min-width:72px">${new Date(t.date_op).toLocaleDateString('fr-FR')}</span>
      <span style="flex:1">${t.libelle || '—'}</span>
      <span style="font-size:.72rem;background:var(--bg3);border-radius:6px;padding:2px 8px;color:var(--text2)">${t.categorie || 'Autres'}</span>
      ${montant}
    </div>`;
  }).join('');
  ...
  try {
    if (!window.XLSX) {
      await new Promise((res, rej) => {
        ...
        s.onload = res; s.onerror = () => rej(new Error('Impossible de charger le lecteur Excel'));
...
async function kmDeleteTrip(id) {
  ...
  const { error } = await sb.from('mileage').delete().eq('id', id);
  if (error) { showToast('Erreur : ' + error.message); return; }
  _allMileageTrips = _allMileageTrips.filter(t => t.id != id);
  renderMileageCalendar();
}
...
// Profils pour l'attestation PDF (personnalisée selon le collaborateur)
const MILEAGE_PDF_PROFILES = {
  'Romain': {
    nom: 'Romain Capdepont', feminin: false, statut: 'gérant',
    qualite: 'Président SASU — Contrat gérant',
    vehicule: 'Mercedes Classe A — Immatriculation DS 591 QS',
    bareme: `le barème kilométrique 2026 correspondant (tranche de + de 20 000 km, avec un coefficient de <strong>0,47 € par km</strong>)`
  },
  'Ketsia': {
    nom: 'Ketsia', feminin: true, statut: 'salariée',
    qualite: 'Responsable Projet — CDI',
    vehicule: 'Véhicule personnel',
    bareme: `le barème retenu par l'entreprise (coefficient de <strong>0,47 € par km</strong>)`
  },
  'Flora': {
    nom: 'Flora Boyer', feminin: true, statut: 'salariée',
    qualite: 'Alternante',
    vehicule: 'Peugeot 207 (4 CV)',
    bareme: `le barème kilométrique 2026, véhicule 4 CV (coefficient de <strong>0,606 € par km</strong>)`
  },
  'Gloria': {
    nom: 'Gloria Dijoux', feminin: true, statut: 'salariée',
    qualite: 'Chargée de développement commercial — CDD',
    vehicule: 'Kia Sportage — Immatriculation FB-608-GW (6 CV)',
    bareme: `le barème retenu par l'entreprise (coefficient de <strong>0,374 € par km</strong>)`
  }
};
...
async function openExportPDFModal() {
  const { data, error } = await sb.from('mileage')
    .select('trip_date, user_name')
    .eq('is_utility', false)
    .order('trip_date', { ascending: false });
  if (error) { showToast('Erreur : ' + error.message); return; }
  _exportPDFTrips = data || [];

  // Sélecteur de collaborateur (pré-sélection : personne affichée sur la page, sinon l'utilisateur connecté)
  const sel = document.getElementById('exportPDF-person');
  if (sel) {
    sel.innerHTML = MILEAGE_MEMBERS.map(m => `<option value="${m.name}">${(MILEAGE_PDF_PROFILES[m.name] || {}).nom || m.name}</option>`).join('');
    let def = null;
    if (typeof mileageCalPerson !== 'undefined' && mileageCalPerson && mileageCalPerson !== 'Tous') def = mileageCalPerson;
    if (!def && typeof mileageFocusPerson !== 'undefined' && mileageFocusPerson) def = mileageFocusPerson;
    if (!def && currentProfile?.name) {
      const me = MILEAGE_MEMBERS.find(m => m.aliases.some(a => currentProfile.name.includes(a)));
      if (me) def = me.name;
    }
    if (def && MILEAGE_MEMBERS.some(m => m.name === def)) sel.value = def;
  }

  exportPDFPersonChanged();
  openModal('exportPDF');
}

function exportPDFPersonChanged() {
  const person = document.getElementById('exportPDF-person')?.value || 'Romain';
  const member = MILEAGE_MEMBERS.find(m => m.name === person);
  const mine = _exportPDFTrips.filter(t => member && member.aliases.some(a => (t.user_name || '').includes(a)));
  const months = [...new Set(mine.map(t => t.trip_date.slice(0, 7)))].sort().reverse();
  const container = document.getElementById('exportPDF-months');
  if (!container) return;

  const MOIS = ['Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'];
  container.innerHTML = months.length === 0
    ? '<p style="color:var(--text2)">Aucun trajet à exporter pour cette personne.</p>'
    : months.map(m => {
        const [y, mo] = m.split('-');
...
async function generateMileagePDF() {
  ...
  const person = document.getElementById('exportPDF-person')?.value || 'Romain';
  const member = MILEAGE_MEMBERS.find(m => m.name === person) || { aliases: [person] };
  const P = MILEAGE_PDF_PROFILES[person] || {
    nom: person, feminin: false, statut: 'salarié', qualite: '—',
    vehicule: 'Véhicule personnel', bareme: `le barème retenu par l'entreprise`
  };
  ...
  const trips = (rawData || []).filter(t => !t.is_utility && member.aliases.some(a => (t.user_name || '').includes(a)) && checked.includes((t.trip_date || '').slice(0, 7)));
  if (!trips.length) { showToast('Aucun trajet trouvé pour cette personne sur les mois sélectionnés.'); return; }
  ...
  const personSlug = person.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  const fileSlug = `attestation-km-${personSlug}-${sorted[0]}-${sorted[sorted.length-1]}`;

  const html = `<!DOCTYPE html>
<html lang="fr"><head><meta charset="UTF-8">
<title>Note de frais kilométriques — ${P.nom}</title>
<style>
  * { box-sizing: border-box; }
  ...
  <div class="infos-bloc">
    <table>
      ...
      <tr><td>${person === 'Romain' ? 'Gérant' : (P.feminin ? 'Salariée' : 'Salarié')} :</td><td>${P.nom}</td></tr>
      <tr><td>Qualité :</td><td>${P.qualite}</td></tr>
      <tr><td>Véhicule :</td><td>${P.vehicule}</td></tr>
      <tr><td>Période :</td><td>${periodHeader}</td></tr>
    </table>
  </div>

  <p class="attestation-text">
    Je soussigné${P.feminin ? 'e' : ''}, <strong>${P.nom}</strong>, ${P.statut} de la société <strong>Pull Up Événements</strong>,
  ...
  ${person === 'Romain' ? `<div class="signature-bloc">
    Au Tampon, le ${dateJour}<br><br>
    <img src="data:image/png;base64," alt="Signature Romain Capdepont" style="height:80px;display:block;margin:8px 0">
    <strong>Romain Capdepont</strong>
  </div>` : `<div class="signature-bloc" style="display:flex;justify-content:space-between;gap:40px;flex-wrap:wrap">
    <div>
      Au Tampon, le ${dateJour}<br><br>
      ${P.feminin ? 'La salariée' : 'Le salarié'}<br>
      <div class="signature-line" style="margin-top:52px"></div>
      <strong>${P.nom}</strong>
    </div>
    <div>
      Pour Pull Up Événements<br><br>
      L'employeur<br>
      <div class="signature-line" style="margin-top:52px"></div>
      <strong>Romain Capdepont</strong>
    </div>
